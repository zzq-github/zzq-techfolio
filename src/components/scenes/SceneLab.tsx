...
function SceneViewport({ kind, paused, onPause }: { kind: SceneKind; paused: boolean; onPause: () => void }) {
  ...
  return (
    <div className="scene-lab-body">
      <div className="simulation-stage">
        ...
        {status !== 'ready' && (
          <div className="simulation-fallback" role="status">
            ...
            <strong>{status === 'error' ? t('当前设备暂时无法显示三维场景') : t('正在准备三维场景')}</strong>
            <p>
              {status === 'error'
                ? t('可继续浏览下方项目说明，或开启浏览器硬件加速后重试。')
                : t('模型由本地参数生成，无需加载外部地图服务。')}
            </p>
            {status === 'error' && (
      ...
      <aside className="simulation-panel" aria-label={t('{name}演示参数', { name: t(definition.name) })}>
        ...
        <div className="simulation-controls">
          <button type="button" onClick={onPause} disabled={status !== 'ready'} aria-pressed={paused}>
            {paused ? <Play size={15} /> : <Pause size={15} />} {paused ? t('播放动画') : t('暂停动画')}
          </button>
          <button
            type="button"
          ...
          >
            <Hand size={15} />
            {interactive ? t('退出拖动') : t('拖动视角')}
          </button>
        </div>
        <p className="simulation-hint">
          {interactive
            ? t('在场景内拖动旋转。退出拖动后，可正常滑动页面。')
            : t('点击「拖动视角」自由观察，或使用画面下方按钮。')}
        </p>
  ...
  return (
    <div className="scene-lab" id="scene-lab" tabIndex={-1} aria-labelledby="scene-lab-title">
      ...
      <div className="scene-lab-note">
        ...
        <p> {t('所有模型、路线和参数均为本地合成，仅展示三维交互能力，不对应真实项目数据或运行状态。')} </p>
      </div>
    </div>
  )
}
